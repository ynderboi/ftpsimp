...
Настройки сохраняются в каталоге конфигурации пользователя (`…/ftpsimp/config.json`). Корневую папку можно сменить в веб-интерфейсе → settings **только открыв UI на том же компьютере**.

После запуска откройте на другом устройстве `http://<LAN-IP>:8080` и введите PIN из консоли.
